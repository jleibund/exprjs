...
//                var obj = st[node.object.name];
        ...
        CallExpression:function(node, st){
            ...
            // this is a raw function defined at root
            if (func){
                var realFunc = null;
                if (st.refs){
                    var len = st.refs.length;
                    for (var i= 0;i<len; i++){
                        var ref = st.refs[i][func];
                ...
                if (found > 1) throw new Error('environment has duplicate functions named '+func);
                node.value = realFunc.apply(st,args);
            } else {
                // lets look
                var calleeFunc = node.callee.property.value;
                var calleeName = node.callee.property.name;
                var calleeObj = node.callee.object.value;
                if (!realFunc && calleeObj && calleeFunc && typeof calleeFunc == 'function') {
                    node.value = calleeObj[calleeName].apply(calleeObj,args)
                }
            }
